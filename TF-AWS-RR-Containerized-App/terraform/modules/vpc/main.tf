...
# Webapp Private Subnets (App Tier - Frontend)
resource "aws_subnet" "webapp" {
  for_each = var.webapp_subnet_cidrs
...
resource "aws_route_table" "webapp" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.environment}-${var.project}-${each.key}-webapp-rtb"
      Tier = "webapp"
    }
  )
}

# Route for Webapp Subnets to NAT GW
resource "aws_route" "webapp_nat" {
  /*
  # for_each               = var.enable_nat_gateway ? var.web_subnet_cidrs : {} # For multiple NAT GWs
  route_table_id         = aws_route_table.web[each.key].id
  */
  for_each = aws_route_table.webapp
...
resource "aws_route_table_association" "webapp" {
  for_each       = var.webapp_subnet_cidrs
  subnet_id      = aws_subnet.webapp[each.key].id
  route_table_id = var.single_nat_gateway ? values(aws_route_table.webapp)[0].id : aws_route_table.webapp[each.key].id
}
